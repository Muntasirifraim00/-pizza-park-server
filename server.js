...
app.get("/", (req, res) => {
  res.send("server is runningfasterrrrrrrrrrr ");
});

const uri = `mongodb+srv://${process.env.DB_USERNAME}:${process.env.DB_PASSWORD}@cluster0.zo1jc.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const database = client.db("muntasir");
